...
function puts(error, stdout, stderr){ 
  ...
  fs.readFile(pwd+'/arguments.txt',function(err,buf){
	if(err){
	    ...
	    var command = min+' '+hour+' '+data.day+' '+data.month+' '+data.week+' '+pwd+'/duplicityScript.sh '+data.accessKey+' '+data.secretKey+' '+data.backUpPassword+' '+data.bucket+' '+data.directoryToBackUp +' >> ' + pwd+'/backupLog'+'\n';
		...
		if(err)throw err;
		    ...
		    fs.write(fd,command,undefined,undefined,function(err,written){
		   	if (err) throw err;
			exec('cat ~/BL/scheduler | crontab -', puts);
